...
//http://tccufra.000webhostapp.com/
//localarduino.000webhostapp.com/

#define rxPin 2
#define txPin 3
SoftwareSerial sim800L(rxPin,txPin);

const int Butao_pin = 6;
char buzzer_pin = 7;
int estadochave = 0;
...
void setup()
{
  ...
  pinMode(Butao_pin, INPUT_PULLUP); // set arduino pin to input pull-up mode
...
int buttonState = digitalRead(Butao_pin); // read new state

if (buttonState == LOW) {
    Serial.println("O botão está sendo pressionado");
    digitalWrite(buzzer_pin, HIGH); // turn on
     for (estadochave = 1500; estadochave < 18000; estadochave += 1){
    tone(buzzer_pin, estadochave, 1000);
      }for (estadochave = 1500; estadochave < 18000; estadochave += 1){
    tone(buzzer_pin, estadochave, 1000);
      }
  ...
  if (buttonState == HIGH) {
    Serial.println("O botão não é pressionado");
...
int sendGpsToServer()
{
    ...
    if(true){
      ...
      String url, temp;
      url = "http://localarduino.000webhostapp.com/gpsdata.php?lat=";
      url += latitude;
      url += "&lng=";
      url += longitude;
